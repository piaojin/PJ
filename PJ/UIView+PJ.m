...
/**
 *  获取当前view的NextReponder,直到找到类名为className的responder
 */
- (id)getNextResponderWithClassName:(NSString *)className{
    UIResponder *next = [self nextResponder];
    do {
        if ([next isKindOfClass:NSClassFromString(className)]) {
            return next;
        }
        next = [next nextResponder];
    } while (next != nil);
